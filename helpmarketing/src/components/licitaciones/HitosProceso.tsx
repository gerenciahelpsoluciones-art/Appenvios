    ...
    <form onSubmit={handleSubmit} style={{
      ...
      {!hito && (
        <div style={{ marginBottom: '0.75rem' }}>
          <button
            type="button"
            onClick={() => setUseSugerido(!useSugerido)}
            style={{
              ...
              {HITOS_SUGERIDOS.map(s => (
                <button
                  ...
                  style={{
                    ...
                    background: nombre === s ? 'rgba(245,158,11,0.2)' : 'rgba(255,255,255,0.05)',
                    border: '1px solid var(--glass-border)',
                    borderRadius: '4px',
                    color: nombre === s ? '#fbbf24' : 'hsl(var(--text-muted))',
                    fontSize: '0.72rem',
                    cursor: 'pointer',
                  }}
                >
      ...
      <div style={{ display: 'flex', gap: '0.5rem', marginTop: '1rem' }}>
        <button type="submit" style={{
          padding: '0.5rem 1.5rem', background: '#f59e0b', border: 'none',
...
export function HitosProceso() {
  ...
  useEffect(() => {
    ...
    if (lics.length > 0 && !selectedLicitId) {
      setSelectedLicitId(lics[0].id);
    }
  ...
  useEffect(() => {
    if (selectedLicitId) {
      setHitos(getHitos(selectedLicitId));
    } else {
      setHitos(getHitos());
    }
  }, [selectedLicitId]);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
      ...
      <div style={{ ...cardStyle, display: 'flex', alignItems: 'center', gap: '1rem', flexWrap: 'wrap' }}>
        ...
        <button
          onClick={() => { setEditingHito(undefined); setShowForm(!showForm); }}
          disabled={!selectedLicitId}
          style={{
            ...
            borderRadius: '8px',
            color: selectedLicitId ? '#fbbf24' : 'hsl(var(--text-muted))',
            fontWeight: 700,
            cursor: selectedLicitId ? 'pointer' : 'not-allowed',
            fontSize: '0.85rem',
          }}
        >
          ➕ Nuevo Hito
      ...
      {showForm && selectedLicit && (
        <HitoForm
          licitId={selectedLicit.id}
          licitNombre={selectedLicit.entidad_nombre}
          hito={editingHito}
          onSave={handleSaved}
      ...
      {hitosOrdenados.length > 0 ? (
            ...
            {hitosOrdenados.map((hito, idx) => {
              ...
              return (
                <div key={hito.id} style={{
                  display: 'flex', gap: '1rem', marginBottom: idx < hitosOrdenados.length - 1 ? '1.5rem' : 0,
                  paddingLeft: '3rem', position: 'relative',
                }}>
                  {/* Timeline dot */}
                  <div style={{
                    position: 'absolute',
                    ...
                    fontSize: '0.75rem',
                    boxShadow: `0 0 0 4px ${isOverdue ? '#ef444420' : cfg.color + '20'}`,
                  }}>
                  ...
                  <div style={{
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '0.5rem' }}>
                      <div>
                        ...
                        {hito.descripcion && (
                          <p style={{ margin: '0.2rem 0 0', fontSize: '0.78rem', color: 'hsl(var(--text-muted))' }}>
                            {hito.descripcion}
                          </p>
                        )}
                      </div>
                      ...
                      <div style={{ display: 'flex', gap: '0.4rem', alignItems: 'center' }}>
                        <select
                          value={hito.estado}
                          onChange={e => handleEstadoChange(hito, e.target.value as HitoEstado)}
                        ...
                        <button
                          onClick={() => { setEditingHito(hito); setShowForm(true); }}
                          style={{
                            padding: '0.2rem 0.4rem',
                    ...
                    {hito.notas && (
                      <p style={{ margin: '0.35rem 0 0', fontSize: '0.75rem', color: 'hsl(var(--text-muted))', fontStyle: 'italic' }}>
                        💬 {hito.notas}
                      </p>
                    )}
                  </div>
        ...
        <div style={{
          ...cardStyle, textAlign: 'center', padding: '3rem', color: 'hsl(var(--text-muted))',
        }}>
