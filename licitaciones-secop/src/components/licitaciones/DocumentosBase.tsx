...
import type { DocumentoBase, Licitacion } from '../../types/licitaciones.types';
...
const TIPOS: { id: DocumentoBase['tipo']; label: string; icon: string }[] = [
  { id: 'carta_presentacion', label: 'Carta de Presentación', icon: '✉️' },
  { id: 'experiencia',        label: 'Experiencia Relevante', icon: '📋' },
  { id: 'propuesta_tecnica',  label: 'Propuesta Técnica',     icon: '⚙️' },
  { id: 'propuesta_economica',label: 'Propuesta Económica',   icon: '💰' },
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
      <div style={cardStyle}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1rem' }}>
          <div>
            <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: 600, color: 'hsl(var(--text-muted))', marginBottom: '0.35rem' }}>Licitación</label>
            <select style={inputStyle} value={selectedLicitId} onChange={e => setSelectedLicitId(e.target.value)}>
          ...
          <div>
            <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: 600, color: 'hsl(var(--text-muted))', marginBottom: '0.35rem' }}>Tipo de Documento</label>
            <select style={inputStyle} value={selectedTipo} onChange={e => setSelectedTipo(e.target.value as DocumentoBase['tipo'])}>
        ...
        {selectedLicit && (
          <div style={{ padding: '0.75rem', background: 'rgba(59,130,246,0.08)', borderRadius: '8px', marginBottom: '1rem', fontSize: '0.8rem', color: 'hsl(var(--text-muted))' }}>
          ...
          {TIPOS.map(t => (
            <button key={t.id} onClick={() => setSelectedTipo(t.id)}
              style={{
                ...
                borderRadius: '6px', color: selectedTipo === t.id ? '#10b981' : 'hsl(var(--text-muted))',
                cursor: 'pointer', fontSize: '0.78rem', fontWeight: selectedTipo === t.id ? 700 : 400, boxShadow: 'none',
              }}>
              {t.icon} {t.label}
            </button>
          ))}
        ...
        <button onClick={handleGenerar} disabled={generating || !selectedLicitId}
          style={{
            ...
            cursor: generating || !selectedLicitId ? 'not-allowed' : 'pointer', fontSize: '0.9rem', boxShadow: 'none',
          }}>
      ...
      {viewDoc && (
        <div style={cardStyle}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
            <h3 style={{ margin: 0, fontSize: '0.95rem', fontWeight: 700, color: '#10b981' }}>📄 {viewDoc.titulo}</h3>
            <div style={{ display: 'flex', gap: '0.5rem' }}>
              <button onClick={() => handleCopy(viewDoc.contenido)}
                style={{ padding: '0.4rem 0.9rem', background: copied ? 'rgba(16,185,129,0.2)' : 'rgba(255,255,255,0.05)', border: '1px solid var(--glass-border)', borderRadius: '6px', color: copied ? '#10b981' : 'hsl(var(--text-muted))', cursor: 'pointer', fontSize: '0.78rem', fontWeight: 600, boxShadow: 'none' }}>
                {copied ? '✅ Copiado' : '📋 Copiar'}
              </button>
              <button onClick={() => setViewDoc(null)}
                style={{ padding: '0.4rem 0.6rem', background: 'transparent', border: '1px solid var(--glass-border)', borderRadius: '6px', color: 'hsl(var(--text-muted))', cursor: 'pointer', fontSize: '0.78rem', boxShadow: 'none' }}>
                ✕
              </button>
            </div>
          </div>
          <pre style={{
            margin: 0, padding: '1rem', background: 'rgba(0,0,0,0.3)', borderRadius: '8px',
            fontSize: '0.82rem', color: 'hsl(var(--text-main))', whiteSpace: 'pre-wrap', wordBreak: 'break-word',
            maxHeight: '500px', overflowY: 'auto', lineHeight: 1.6, fontFamily: 'Inter, sans-serif',
          }}>
            {viewDoc.contenido}
          </pre>
        </div>
        ...
        <div style={cardStyle}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
            {documentos.map(doc => {
              ...
              return (
                <div key={doc.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0.75rem', background: 'rgba(255,255,255,0.03)', borderRadius: '8px', border: '1px solid var(--glass-border)' }}>
                  ...
                  <div style={{ display: 'flex', gap: '0.4rem' }}>
                    <button onClick={() => setViewDoc(doc)}
                      style={{ padding: '0.35rem 0.7rem', background: 'rgba(59,130,246,0.1)', border: '1px solid rgba(59,130,246,0.2)', borderRadius: '6px', color: '#60a5fa', cursor: 'pointer', fontSize: '0.75rem', fontWeight: 600, boxShadow: 'none' }}>
                      👁️ Ver
                    </button>
                    <button onClick={() => handleDelete(doc.id)}
                      style={{ padding: '0.35rem 0.7rem', background: 'transparent', border: '1px solid rgba(239,68,68,0.2)', borderRadius: '6px', color: '#f87171', cursor: 'pointer', fontSize: '0.75rem', boxShadow: 'none' }}>
                      🗑️
                    </button>
                  </div>
                </div>
