...
import type { Licitacion, EmpresaPerfil, TelegramLicitConfig } from '../../types/licitaciones.types';
...
import { getLicitacionesGuardadas, updateLicitacion, getEmpresaPerfil, saveEmpresaPerfil, getRupData } from './store';
...
export function MatchScoreIA() {
  ...
  const handleAnalizarTodas = async () => {
    for (const l of licitaciones) {
      if (l.match_score === undefined) await handleAnalizar(l);
    }
  };

  const handleSavePerfil = () => {
    try {
      const parsed = JSON.parse(perfilDraft) as EmpresaPerfil;
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
      <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap' }}>
        <button
          ...
          style={{ padding: '0.55rem 1.25rem', background: 'rgba(139,92,246,0.2)', border: '1px solid rgba(139,92,246,0.4)', borderRadius: '8px', color: '#a78bfa', fontWeight: 700, cursor: 'pointer', fontSize: '0.85rem', boxShadow: 'none' }}
        >
        ...
        <button
          ...
          style={{ padding: '0.55rem 1.25rem', background: 'rgba(59,130,246,0.15)', border: '1px solid rgba(59,130,246,0.3)', borderRadius: '8px', color: '#60a5fa', fontWeight: 700, cursor: 'pointer', fontSize: '0.85rem', boxShadow: 'none' }}
        >
          🏢 Perfil Empresa
        </button>
        <button
          onClick={() => setShowTelegramConfig(!showTelegramConfig)}
          style={{ padding: '0.55rem 1.25rem', background: 'rgba(59,130,246,0.1)', border: '1px solid rgba(59,130,246,0.2)', borderRadius: '8px', color: '#93c5fd', fontWeight: 700, cursor: 'pointer', fontSize: '0.85rem', boxShadow: 'none' }}
        >
          📬 Telegram
        </button>
      </div>
      ...
      {showPerfilEditor && (
        <div style={cardStyle}>
          ...
          <textarea value={perfilDraft} onChange={e => setPerfilDraft(e.target.value)} rows={16}
            style={{ ...inputStyle, fontFamily: 'Fira Code, monospace', fontSize: '0.78rem', resize: 'vertical' }} />
          {perfilError && <p style={{ color: '#f87171', fontSize: '0.8rem', marginTop: '0.5rem' }}>{perfilError}</p>}
          <div style={{ display: 'flex', gap: '0.5rem', marginTop: '0.75rem' }}>
            <button onClick={handleSavePerfil}
              style={{ padding: '0.5rem 1.25rem', background: '#10b981', border: 'none', borderRadius: '8px', color: 'white', fontWeight: 700, cursor: 'pointer', fontSize: '0.85rem', boxShadow: 'none' }}>
              💾 Guardar Perfil
            </button>
            <button onClick={() => setShowPerfilEditor(false)}
      ...
      {showTelegramConfig && (
        <div style={cardStyle}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
            <div>
              <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: 600, color: 'hsl(var(--text-muted))', marginBottom: '0.35rem' }}>Bot Token</label>
              <input style={inputStyle} type="password" placeholder="1234567890:ABC..."
                value={telegramCfg.bot_token} onChange={e => setTelegramCfg(p => ({ ...p, bot_token: e.target.value }))} />
            </div>
            <div>
              <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: 600, color: 'hsl(var(--text-muted))', marginBottom: '0.35rem' }}>Chat ID</label>
              <input style={inputStyle} placeholder="-100123456789 o @canal"
                value={telegramCfg.chat_id} onChange={e => setTelegramCfg(p => ({ ...p, chat_id: e.target.value }))} />
            </div>
            <div>
              <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: 600, color: 'hsl(var(--text-muted))', marginBottom: '0.35rem' }}>Score mínimo para notificar</label>
              <input style={inputStyle} type="number" min="0" max="100"
                value={telegramCfg.score_minimo} onChange={e => setTelegramCfg(p => ({ ...p, score_minimo: Number(e.target.value) }))} />
            </div>
          </div>
          <div style={{ display: 'flex', gap: '1.5rem', marginTop: '0.75rem', flexWrap: 'wrap' }}>
            {([
              ...
              <label key={key} style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', cursor: 'pointer', fontSize: '0.85rem' }}>
                <input type="checkbox" checked={telegramCfg[key] as boolean}
          ...
          {telegramTest && (
            <p style={{ color: telegramTest.startsWith('✅') ? '#10b981' : '#f87171', fontSize: '0.85rem', marginTop: '0.5rem' }}>
              {telegramTest}
            </p>
          )}
          <div style={{ display: 'flex', gap: '0.5rem', marginTop: '0.75rem', flexWrap: 'wrap' }}>
            <button onClick={() => { saveConfig(telegramCfg); setShowTelegramConfig(false); }}
              style={{ padding: '0.5rem 1.25rem', background: '#10b981', border: 'none', borderRadius: '8px', color: 'white', fontWeight: 700, cursor: 'pointer', fontSize: '0.85rem', boxShadow: 'none' }}>
              💾 Guardar
            </button>
            <button onClick={handleTestTelegram}
              style={{ padding: '0.5rem 1.25rem', background: 'rgba(59,130,246,0.2)', border: '1px solid rgba(59,130,246,0.3)', borderRadius: '8px', color: '#60a5fa', fontWeight: 700, cursor: 'pointer', fontSize: '0.85rem', boxShadow: 'none' }}>
              🔔 Probar conexión
            </button>
          </div>
        </div>
      ...
      {licitaciones.length === 0 ? (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(340px, 1fr))', gap: '1rem' }}>
          {licitaciones.map(licit => (
            <div key={licit.id} onClick={() => setSelectedId(selectedId === licit.id ? null : licit.id)}
              style={{
                ...cardStyle, cursor: 'pointer',
                border: selectedId === licit.id ? '1px solid rgba(139,92,246,0.5)' : '1px solid var(--glass-border)',
            ...
            >
              <div style={{ display: 'flex', gap: '1rem', alignItems: 'center', marginBottom: '0.75rem' }}>
                {licit.match_score !== undefined ? (
                  ...
                  <div style={{
                    width: 100, height: 100, borderRadius: '50%', border: '10px solid rgba(255,255,255,0.1)',
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <p style={{ margin: '0.25rem 0', fontSize: '0.75rem', color: 'hsl(var(--text-muted))', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    {licit.descripcion}
                  </p>
              ...
              <button onClick={e => { e.stopPropagation(); void handleAnalizar(licit); }} disabled={analyzing === licit.id}
                style={{
                  width: '100%', padding: '0.4rem', background: 'rgba(139,92,246,0.15)', border: '1px solid rgba(139,92,246,0.3)',
                  borderRadius: '6px', color: '#a78bfa', fontWeight: 700, cursor: analyzing === licit.id ? 'not-allowed' : 'pointer',
                ...
                <div style={{ marginTop: '1rem', borderTop: '1px solid var(--glass-border)', paddingTop: '1rem' }}>
                  ...
                  {licit.match_result.fortalezas.length > 0 && (
                    <div style={{ marginBottom: '0.75rem' }}>
                      <p style={{ margin: '0 0 0.35rem', fontSize: '0.75rem', fontWeight: 700, color: '#10b981' }}>✅ Fortalezas</p>
                      {licit.match_result.fortalezas.map((f, i) => (
                        <p key={i} style={{ margin: '0 0 0.2rem', fontSize: '0.78rem', color: 'hsl(var(--text-main))', paddingLeft: '0.75rem' }}>• {f}</p>
                      ))}
                    </div>
                  )}
                  {licit.match_result.debilidades.length > 0 && (
                    <div style={{ marginBottom: '0.75rem' }}>
                      <p style={{ margin: '0 0 0.35rem', fontSize: '0.75rem', fontWeight: 700, color: '#f87171' }}>⚠️ Puntos débiles</p>
                      {licit.match_result.debilidades.map((d, i) => (
                        <p key={i} style={{ margin: '0 0 0.2rem', fontSize: '0.78rem', color: 'hsl(var(--text-main))', paddingLeft: '0.75rem' }}>• {d}</p>
                      ))}
                    </div>
                  )}
                  {licit.match_result.recomendaciones.length > 0 && (
                    <div>
                      <p style={{ margin: '0 0 0.35rem', fontSize: '0.75rem', fontWeight: 700, color: '#fbbf24' }}>💡 Recomendaciones</p>
                      {licit.match_result.recomendaciones.map((r, i) => (
                        <p key={i} style={{ margin: '0 0 0.2rem', fontSize: '0.78rem', color: 'hsl(var(--text-main))', paddingLeft: '0.75rem' }}>• {r}</p>
                      ))}
                    </div>
                  )}
                </div>
